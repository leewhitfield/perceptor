...
def test_workspace_paths_are_under_root(tmp_path):
    live_mount_root = tmp_path / "live-mounts"
    paths = WorkspacePaths(tmp_path, live_mount_root=live_mount_root)
    case_id = "case-1"

    assert paths.case_dir(case_id) == tmp_path / "cases" / case_id
    assert paths.mounts_dir(case_id) == tmp_path / "cases" / case_id / "mounts"
    assert paths.live_mounts_dir(case_id) == live_mount_root / "cases" / case_id
    assert paths.ewf_mount_dir(case_id) == live_mount_root / "cases" / case_id / "ewf"
    ...
    assert paths.volume_mount_dir(case_id, "part-001") == (
        live_mount_root / "cases" / case_id / "volumes" / "part-001"
    )
...
def test_ensure_case_tree_creates_expected_dirs(tmp_path):
    ...
    assert paths.mounts_dir("case-1").is_dir()
    assert paths.ewf_mount_dir("case-1").is_dir()
    assert (paths.live_mounts_dir("case-1") / "volumes").is_dir()
