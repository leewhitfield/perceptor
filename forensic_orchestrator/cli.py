...
def write_case_report_bundle(
    ...
    *,
    ...
    report_builders: list[tuple[str, str, str, Callable[[], object]]] = [
        ("activity-digest", "json", "Activity digest", lambda: case_activity_digest_report(db, case_id, limit=limit)),
        ...
        ("memory-credentials", "json", "Memory credentials", lambda: credentials()),
        ("memory-disk-correlations", "json", "Memory/disk correlations", lambda: memory_disk()),
        ("memory-support-files", "json", "Memory support files", lambda: memory_support()),
        ("structured-memory", "json", "Structured memory records", lambda: structured_memory_report(db, case_id, limit=limit)),
        ("combined-artifacts", "json", "Combined artifact families", lambda: combined_artifact_family_report(db, case_id, limit=limit, memory_disk_report=memory_disk())),
        ("crash-dump-analysis", "json", "Crash dump analysis", lambda: crash_dump_analysis_report(db, case_id, limit=limit)),
        ("execution", "json", "Execution", lambda: execution_report(db, case_id, limit=limit)),
        ("execution-correlation", "json", "Execution correlation", lambda: execution_correlation_report(db, case_id, limit=limit)),
        ("suspicious-executions", "json", "Suspicious executions", lambda: suspicious()),
        ("investigation-findings", "json", "Investigation findings", lambda: investigation_findings_report(db, case_id, limit=limit, rebuild=False)),
        ("event-interpretation", "json", "High-value event log interpretation", lambda: event_interpretation_report(db, case_id, limit=limit)),
        ("program-provenance", "json", "Program provenance", lambda: program_provenance_report(db, case_id, limit=limit)),
        ("software-footprint-review", "json", "Software footprint review", lambda: software_footprint_review_report(db, case_id, limit=limit)),
        ...
        ("memory-artifacts", "json", "Memory artifact inventory", lambda: memory_artifacts_report(db, case_id, limit=limit)),
        ("deep-recovery-status", "json", "Deep recovery status", lambda: deep_recovery_status_report(db, case_id, limit=limit)),
        ("user-intent", "json", "User intent artifacts", lambda: user_intent()),
        ...
        ("external-storage", "json", "External storage", lambda: external_storage()),
        ("shellbag-external-storage", "json", "Shellbag external storage", lambda: shellbag_storage()),
        ("usb-files", "json", "USB file correlations", lambda: usb_files()),
        ("usb-timeline", "json", "USB timeline", lambda: usb_timeline()),
        ("file-movement-identity", "json", "File movement and identity", lambda: file_identity()),
        ("opened-from-removable-media", "json", "Opened from removable media", lambda: opened_removable()),
        ("opened-from-cloud-storage", "json", "Opened from cloud storage", lambda: opened_cloud()),
        ("cloud-mounts", "json", "Cloud virtual mounts", lambda: cloud_mounts()),
        ("cloud-removable-overlap", "json", "Cloud/removable overlap", lambda: cloud_overlap()),
        ("shortcut-droid-changes", "json", "Shortcut Droid changes", lambda: shortcut_droid_changes_report(db, case_id, limit=limit)),
        ("shortcut-object-tracking", "json", "Shortcut Object ID tracking", lambda: shortcut_object_tracking_report(db, case_id, limit=limit)),
        ("usn-lifecycle", "json", "USN file lifecycle", lambda: usn_file_lifecycle_report(db, case_id, limit=limit)),
        ("recovery-coverage", "json", "Recovery coverage", lambda: recovery_coverage_report(db, case_id, limit=max(limit, 250))),
        ("carve-coverage", "json", "Carve coverage", lambda: carve_coverage_report(db, case_id, limit=max(limit, 250))),
        ("sqlite-inventory", "json", "SQLite carve inventory", lambda: sqlite_inventory_report(db, case_id, limit=limit)),
        ...
        ("processing-decisions", "json", "Processing decisions", lambda: processing_decision_report(db, case_id, limit=limit)),
        ("processing-readiness", "json", "Processing readiness", lambda: readiness()),
        ...
        ("changed-search-packets", "json", "Changed search packets", lambda: changed_packets()),
    ]
    ...
    markdown_export_builders: list[tuple[str, str, Callable[[], str]]] = [
        ...
        ("memory-credentials", "Memory credentials Markdown", lambda: memory_credentials_markdown(credentials())),
        ("memory-disk-correlations", "Memory/disk correlations Markdown", lambda: memory_disk_correlations_markdown(memory_disk())),
        ("memory-support-files", "Memory support files Markdown", lambda: memory_support_files_markdown(memory_support())),
        ("structured-memory", "Structured memory records Markdown", lambda: structured_memory_markdown(structured_memory_report(db, case_id, limit=limit))),
        ("combined-artifacts", "Combined artifact families Markdown", lambda: combined_artifact_family_markdown(combined_artifact_family_report(db, case_id, limit=limit, memory_disk_report=memory_disk()))),
        ("crash-dump-analysis", "Crash dump analysis Markdown", lambda: crash_dump_analysis_markdown(crash_dump_analysis_report(db, case_id, limit=limit))),
        ("execution", "Execution Markdown", lambda: execution_markdown(execution_report(db, case_id, limit=limit))),
        ("suspicious-executions", "Suspicious executions Markdown", lambda: suspicious_executions_markdown(suspicious())),
        ("investigation-findings", "Investigation findings Markdown", lambda: investigation_findings_markdown(investigation_findings_report(db, case_id, limit=limit, rebuild=False))),
        ("program-provenance", "Program provenance Markdown", lambda: program_provenance_markdown(program_provenance_report(db, case_id, limit=limit))),
        ("software-footprint-review", "Software footprint review Markdown", lambda: software_footprint_review_markdown(software_footprint_review_report(db, case_id, limit=limit))),
        ("memory-artifacts", "Memory artifact inventory Markdown", lambda: memory_artifacts_markdown(memory_artifacts_report(db, case_id, limit=limit))),
        ("deep-recovery-status", "Deep recovery status Markdown", lambda: deep_recovery_status_markdown(deep_recovery_status_report(db, case_id, limit=limit))),
        ("user-intent", "User intent artifacts Markdown", lambda: user_intent_artifacts_markdown(user_intent())),
        ("external-storage", "External storage Markdown", lambda: external_storage_markdown(external_storage())),
        ("shellbag-external-storage", "Shellbag external storage Markdown", lambda: shellbag_external_storage_markdown(shellbag_storage())),
        ("file-movement-identity", "File movement and identity Markdown", lambda: file_movement_identity_markdown(file_identity())),
        ("opened-from-removable-media", "Opened from removable media Markdown", lambda: opened_from_removable_media_markdown(opened_removable())),
        ("opened-from-cloud-storage", "Opened from cloud storage Markdown", lambda: opened_from_cloud_storage_markdown(opened_cloud())),
        ("cloud-mounts", "Cloud virtual mounts Markdown", lambda: cloud_mounts_markdown(cloud_mounts())),
        ("cloud-removable-overlap", "Cloud/removable overlap Markdown", lambda: cloud_removable_overlap_markdown(cloud_overlap())),
        ("shortcut-droid-changes", "Shortcut Droid changes Markdown", lambda: shortcut_droid_changes_markdown(shortcut_droid_changes_report(db, case_id, limit=limit))),
        ("shortcut-object-tracking", "Shortcut Object ID tracking Markdown", lambda: shortcut_object_tracking_markdown(shortcut_object_tracking_report(db, case_id, limit=limit))),
        ("usn-lifecycle", "USN file lifecycle Markdown", lambda: usn_file_lifecycle_markdown(usn_file_lifecycle_report(db, case_id, limit=limit))),
        ("carve-coverage", "Carve coverage Markdown", lambda: carve_coverage_markdown(carve_coverage_report(db, case_id, limit=max(limit, 250)))),
        ("sqlite-inventory", "SQLite carve inventory Markdown", lambda: sqlite_inventory_markdown(sqlite_inventory_report(db, case_id, limit=limit))),
        ("processing-decisions", "Processing decisions Markdown", lambda: processing_decision_markdown(processing_decision_report(db, case_id, limit=limit))),
        ("processing-readiness", "Processing readiness Markdown", lambda: processing_readiness_markdown(readiness())),
        ("lead-search-usb-summary", "USB lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("usb"))),
        ("lead-search-execution-summary", "Execution lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("execution"))),
        ("lead-search-cloud-summary", "Cloud lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("cloud"))),
        ("lead-search-browser-summary", "Browser lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("browser"))),
        ("lead-search-documents-summary", "Document lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("documents"))),
        ("lead-search-communications-summary", "Communications lead search summary Markdown", lambda: artifact_lead_search_markdown(lead_search("communications"))),
        ("changed-search-packets", "Changed search packets Markdown", lambda: changed_search_packets_markdown(changed_packets())),
    ]
    for stem, title, builder in markdown_export_builders:
        if not wanted(stem):
            continue
        _progress(progress, f"report bundle write markdown-export name={stem}")
        path = output_dir / f"{stem}.md"
        write_text_output(builder(), str(path))
        written.append({"name": stem, "title": title, "path": str(path), "format": "md", "role": "export"})
